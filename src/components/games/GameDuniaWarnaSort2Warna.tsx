...
export default function GameDuniaWarnaSort2Warna({
  childId,
  childName,
  colors,
  onSessionComplete,
  ...
  useEffect(() => {
    ...
    async function loadContent() {
      ...
      setPutaran(dipilih)
      if (dipilih) {
        setWarnaBendaAktif(dipilih.warna_sort[0])
      }
      setLoading(false)
    }
  ...
  const simpanRepetisiLog = useCallback(
    async (repetisiKeSelesai: number, targetValue: string, jumlahSalah: number, diselesaikan: boolean) => {
      ...
      await supabase.from('repetisi_log').insert({
        ...
        mechanic_level_key: 'sort_2_warna',
  ...
  return (
    <div
      style={{ backgroundColor: colors.background }}
      className="relative min-h-[480px] flex flex-col items-center justify-between gap-8 px-6 py-10 overflow-hidden select-none"
    >
      <style>{`

...
function BendaMengambang({ nama, hex, size }: { nama: string; hex: string; size: number }) {
  const bentuk = BENTUK_OBJEK[nama] ?? 'bulat'
  return (
...
function pilihPutaranBerbobot(putaranList: Putaran[], riwayat: RiwayatWarna[]): Putaran | null {
  ...
  const bobotList = putaranList.map((p) => {
    if (p.warna_sort.includes(warnaSortTerakhir ?? '')) {
      return 0
    }
...
export default function GameDuniaWarnaSort6Warna({
  childId,
  childName,
  colors,
  onSessionComplete,
  ...
  // BUG FIX: tidak ada mekanisme sebelumnya yang menaikkan minggu aktif
  // anak - currentWeek hanya dihitung dari week_number TERBESAR yang
  // sudah pernah tersimpan, tapi tidak ada yang pernah menyimpan
  // week_number lebih besar dari yang sudah ada (sirkular). Variabel ini
  // menyimpan target minggu berikutnya & variant gate minggu itu, untuk
  // dipakai menulis SATU sesi "penanda pembuka minggu" begitu Tantangan
  // minggu ini selesai - lihat handleTapBonus & bukaMingguBerikutnya().
  ...
  useEffect(() => {
    ...
    async function loadContent() {
      ...
      // BUG FIX: cari minggu berikutnya (minggu tertinggi di tema SAAT
      // INI, +1) dan tema yang week_range-nya mencakup minggu itu.
      // Disiapkan di awal (saat mount), dipakai nanti saat Tantangan
      // selesai - supaya tidak perlu fetch ulang di tengah interaksi.
      ...
      const nextTheme = (themesData as Array<{ id: string; week_range: number[] }>).find(
        (t) => t.week_range.includes(calonNextWeek)
      )

      if (nextTheme) {
        // mechanicData sudah di-fetch di atas dengan level_key yang SAMA
        // (sort_6_warna) - tidak perlu fetch ulang.
      ...
      // Jika nextTheme tidak ditemukan (currentWeek sudah di minggu
      // terakhir/minggu 4 review_gate), nextWeekNumber tetap null -
      // tidak ada minggu lagi untuk dibuka, Dunia Warna selesai total.
      ...
      if (dipilih) {
        setWarnaBendaAktif(
          dipilih.warna_sort[Math.floor(Math.random() * dipilih.warna_sort.length)]
        )
      }
      setLoading(false)
    }
  ...
  const simpanRepetisiLog = useCallback(
    async (repetisiKeSelesai: number, targetValue: string, jumlahSalah: number, diselesaikan: boolean) => {
      ...
      await supabase.from('repetisi_log').insert({
        ...
        mechanic_level_key: 'sort_6_warna',
  ...
  function handleTapKeranjang(warnaKeranjang: string) {
    ...
    if (isBenar) {
      ...
      setTimeout(() => {
        ...
        if (isLastBenda) {
          ...
          setRiwayatWarnaSesi((prev) => [
            {
              warna_key: warnaBendaAktif,
              correct_items: newCorrectCount,
              total_items: jumlahObjekWarnaIni,
          ...
          if (repetisiKe < TARGET_REPETISI) {
            ...
            if (putaranBaru) {
              setWarnaBendaAktif(
                putaranBaru.warna_sort[Math.floor(Math.random() * putaranBaru.warna_sort.length)]
              )
            }
            setRepetisiKe((r) => r + 1)
            setBendaIndex(0)
          ...
          setKeranjangState({})
          setWarnaBendaAktif(
            putaran.warna_sort[Math.floor(Math.random() * putaran.warna_sort.length)]
          )
        }
      }, 700)
    } else {
      ...
      setTimeout(() => {
        setKeranjangState((s) => ({ ...s, [warnaKeranjang]: 'idle' }))
      }, 400)
    }
  }
        ...
        if (isLastBonus) {
          bukaMingguBerikutnya().then(() => {
            onSessionComplete?.({
              ...
              totalItems: putaran?.warna_sort.length ?? 0,
              levelTerbuka: true,
            })
          })
        } else {
  ...
  // BUG FIX: ini fungsi yang sebelumnya TIDAK ADA SAMA SEKALI - tanpa
  // ini, currentWeek di SEMUA komponen game (termasuk tingkat 1-3 minggu
  // berikutnya) tidak akan pernah naik, karena currentWeek dihitung dari
  // week_number TERBESAR yang sudah tersimpan di game_sessions, dan tidak
  // ada yang pernah menyimpan angka lebih besar dari minggu yang sedang
  // dimainkan. Fungsi ini menulis SATU baris "penanda pembuka minggu" -
  // total_items=0, correct_items=0 (TIDAK memengaruhi statistik akurasi
  // anak) - dengan week_number = minggu berikutnya, supaya currentWeek
  // di sesi BERIKUTNYA (kapan pun, tingkat apa pun) otomatis terbaca naik.
  async function bukaMingguBerikutnya() {
    if (!nextWeekNumber || !nextWeekVariantId) return // sudah di minggu terakhir
    await supabase.from('game_sessions').insert({
      child_id: childId,
      game_key: 'dunia_warna',
      content_variant_id: nextWeekVariantId,
  ...
  async function simpanSesiNormal(finalCorrectCount: number, putaranSesi: Putaran, warnaKey: string) {
    ...
    await supabase.from('game_sessions').insert({
      ...
      total_items: putaranSesi.objek[warnaKey]?.length ?? 1,
  ...
  return (
    <div
      style={{ backgroundColor: colors.background }}
      className="relative min-h-[480px] flex flex-col items-center justify-between gap-6 px-4 py-8 overflow-hidden select-none"
    >
      <style>{`
        @keyframes benda-float {
          0%, 100% { transform: translateY(0) rotate(0deg); }
          50% { transform: translateY(-6px) rotate(-2deg); }
        }
        .benda-mengambang { animation: benda-float 3.2s ease-in-out infinite; }
        @keyframes keranjang-idle-bounce {
        ...
        @media (prefers-reduced-motion: reduce) {
          .benda-mengambang, .keranjang-idle, .keranjang-wrong, .keranjang-correct, .bonus-border { animation: none; }
        }
      `}</style>
      ...
      <div className="flex items-center justify-center" style={{ minHeight: 90 }}>
        <BendaMengambang
          nama={objekBendaAktif}
          hex={WARNA_HEX[warnaBendaAktif] ?? '#C9C7BE'}
          size={80}
        />
      </div>
        ...
        {urutanKeranjang.map((k) => (
          <div key={k.warna} className="flex items-center justify-center">
            <KeranjangGrid
              ...
              hex={k.hex}
              size={82}
              state={keranjangState[k.warna] ?? 'idle'}
              onTap={() => handleTapKeranjang(k.warna)}
            />
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col items-center gap-3">
        <div className="flex gap-2" aria-hidden="true">
          {(faseBonus
            ...
            : Array.from({ length: putaran?.objek[warnaBendaAktif]?.length ?? 1 })
          ).map((_, i) => (
            <span
              key={i}
              style={{
                backgroundColor:
                  i <= (faseBonus ? bonusIndex : bendaIndex)
                    ? faseBonus
                      ? colors.accent
                      : colors.primary
                    : colors.secondary,
